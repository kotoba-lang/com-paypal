(ns paypal-compat.main
  "Kotodama WASM entrypoint for the PayPal clean-room actor (L5) — Clojure port.
  ...
  facts (`paypal.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "paypal")
(def tier "L5")
...
(def entity-specs
  [{:entity "Order"   :plural "orders"   :id-prefix "paypal_ord"
    :fields [:status :intent]              :required [:status :intent]
    :coerce {}                       :refs {}}
   {:entity "Authorization" :plural "authorizations" :id-prefix "paypal_aut"
    :fields [:status :createTime :updateTime] :required [:status :createTime]
    :coerce {}                       :refs {}}
   {:entity "Capture"     :plural "captures"     :id-prefix "paypal_cap"
    :fields [:status :invoiceId :customId]   :required [:status]
    :coerce {}                       :refs {}}
   {:entity "Refund"  :plural "refunds"  :id-prefix "paypal_ref"
    :fields [:status :invoiceId :customId]  :required [:status]
    :coerce {}                       :refs {}}
   {:entity "Plan"    :plural "plans"    :id-prefix "paypal_pla"
    :fields [:productId :status :name :description] :required [:status :name]
    :coerce {}                       :refs {}}
   {:entity "Subscription" :plural "subscriptions" :id-prefix "paypal_sub"
    :fields [:status :planId :createTime :startTime] :required [:status :createTime]
    :coerce {}                       :refs {:planId "Plan"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"paypal.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "paypal-compat" :tier tier :entities entities} 200])
...
(defn start! [] :paypal-compat/ready)
